...
export default function Contact() {
  ...
  // Build subject/body for mailto & Gmail
  const subject = useMemo(() => {
    const name = [formData.firstName, formData.lastName].filter(Boolean).join(" ");
    return `Portfolio contact from ${name || "your website"}`;
  ...
  const mailtoHref = useMemo(() => {
    return `mailto:parthgod0708@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  }, [subject, body]);

  const gmailHref = useMemo(() => {
    return `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(
      "parthgod0708@gmail.com"
    )}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  }, [subject, body]);

  const canSend = formData.email.trim().length > 0 && formData.message.trim().length > 0;

  const onSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
    e.preventDefault();
    if (!canSend) return;
    // Open default mail client with the composed message
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 -z-10 pointer-events-none" aria-hidden>
        <div
          className="absolute top-16 right-20 w-32 h-32 rounded-full blur-3xl"
          style={{ background: "radial-gradient(circle, rgba(99,102,241,.25), transparent 60%)" }}
        />
        <div
          className="absolute bottom-24 left-16 w-56 h-56 rounded-full blur-3xl"
          style={{ background: "radial-gradient(circle, rgba(236,72,153,.18), transparent 60%)" }}
        />
        <div
          className="absolute inset-0"
          ...
          <div className="rounded-2xl border border-white/15 bg-white/5 backdrop-blur-sm p-6 animate-scaleIn transition-[transform,box-shadow,border-color] duration-300 hover:-translate-y-1 hover:shadow-md hover:border-white/25">
            ...
            <form onSubmit={onSubmit} noValidate className="space-y-4">
              ...
              <button
                type="submit"
                disabled={!canSend}
                className={`w-full inline-flex items-center justify-center gap-2 rounded-md px-4 py-2 font-medium text-white
                            bg-gradient-to-r from-indigo-500 to-fuchsia-500
                            hover:from-indigo-500/90 hover:to-fuchsia-500/90
                            transition-transform ${canSend ? "hover:scale-[1.02]" : "opacity-60 cursor-not-allowed"}`}
                aria-disabled={!canSend}
                title={!canSend ? "Enter your email and a message" : "Open your email app"}
              >
                <Send size={16} />
                Send Message
              </button>
              ...
              <div className="text-center">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className={`text-xs underline underline-offset-4 ${canSend ? "text-white/80 hover:text-white" : "text-white/40 pointer-events-none"}`}
                >
                  Use Gmail instead
                </a>
              </div>
